...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchData() {
        ...
        if (!listsRes.ok || !tasksRes.ok) throw new Error("Ошибка загрузки данных");
    ...
    function processData(data, filter) {
        ...
        return Object.entries(tasksByList).map(([listId, count]) => ({
            value: count,
    ...
    function renderChart(data) {
        ...
        const option = {
            ...
            legend: { top: '5%', left: 'center', textStyle: { color: '#333' } },
            series: [{
                name: 'Распределение',
                type: 'pie',
                radius: ['40%', '70%'],
                ...
                data: data.length > 0 ? data : [{value: 1, name: 'Нет данных', itemStyle: {color: '#ccc'}}]
            }]
        };
